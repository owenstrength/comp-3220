...
(define interp
  (lambda (stmt myEnv)
       (cond
        ...
        ;; Added for if statements
        ((eqv? (car stmt) 'if) 
          (if (bool-exp myEnv (cadr stmt))
              (process (cadr (caddr stmt)) myEnv)
              myEnv))
        ;; Added for while loops
        ((eqv? (car stmt) 'while) 
          (if (bool-exp myEnv (cadr stmt))
              (let ((new-env (process (cadr (caddr stmt)) myEnv)))
                (interp stmt new-env))
              myEnv))
        ( else (display "\nI saw something I didn't understand.")))))

;; bool-exp : environment, boolean expression -> boolean
(define bool-exp
  (lambda (myEnv b)
    (cond
      ((eq? b #t) #t)
      ((eq? b #f) #f)
      ((integer? b) (not (= b 0))) ;; non-zero is true
      ((symbol? b) (bool-exp myEnv (apply-env myEnv b)))
      ((eqv? (car b) '>) (> (exp myEnv (cadr b)) (exp myEnv (caddr b))))
      ((eqv? (car b) '<) (< (exp myEnv (cadr b)) (exp myEnv (caddr b))))
      ((eqv? (car b) '&) (and (bool-exp myEnv (cadr b)) (bool-exp myEnv (caddr b))))
      (else (display "\nI saw a boolean expression I didn't understand.") #f))))
